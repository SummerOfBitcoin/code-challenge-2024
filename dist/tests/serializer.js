...
class Serializer {
    ...
    writeUint32(value) {
        this.data.push((value & 0xFF000000) >> 24, (value & 0x00FF0000) >> 16, (value & 0x0000FF00) >> 8, (value & 0x000000FF));
    }
    writeVarUint(value) {
        if (value < 0xFD) {
...
function main() {
    ...
    // input 0
    ss.writeHash(buffer_1.Buffer.from(tx.vin[0].txid, 'hex')); // prevout hash
    ss.writeUint32(tx.vin[0].vout); // prevout index
    // input script
    const inputScript = buffer_1.Buffer.from(tx.vin[0].witness[0], 'hex');
    ...
    tx.vout.forEach(output => {
        ...
        const outputScript = buffer_1.Buffer.from(output.scriptpubkey, 'hex');
    ...
    const txHash = (0, crypto_1.createHash)('sha256').update(buffer_1.Buffer.from(hexString, 'hex')).digest();
    console.log('Transaction Hash:', txHash.toString('hex'));
    console.log("id", (0, crypto_1.createHash)('sha256').update(txHash.toString('hex')).digest('hex'));
}
main();
