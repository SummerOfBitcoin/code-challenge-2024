...
exports.executeScript = exports.OP_CODES = void 0;
exports.OP_CODES = {
    OP_ADD: 0x93,
...
const opcodes = [
    {
        name: "OP_DUP",
        opcode: exports.OP_CODES.OP_DUP,
        function: (stack) => {
            if (stack.length === 0)
                throw new Error("OP_DUP: Stack empty");
    ...
    {
        name: "OP_HASH160",
        opcode: exports.OP_CODES.OP_HASH160,
        function: (stack) => {
            if (stack.length === 0)
                throw new Error("OP_HASH160: Stack empty");
            const top = stack.pop();
    ...
    {
        name: "OP_EQUALVERIFY",
        opcode: exports.OP_CODES.OP_EQUALVERIFY,
        function: (stack) => {
            if (stack.length < 2)
                throw new Error("OP_EQUALVERIFY: Stack too small");
            const a = stack.pop();
            const b = stack.pop();
            if (a !== b)
                throw new Error("OP_EQUALVERIFY: Verification failed");
            return stack;
        },
    },
    {
        name: "OP_CHECKSIG",
        opcode: exports.OP_CODES.OP_CHECKSIG,
        function: (stack, tx, inputIndex) => {
            if (stack.length < 2)
                throw new Error("OP_CHECKSIG: Stack too small");
...
function executeScript(script, tx, inputIndex) {
    let stack = [];
...
function verifySignature(tx, inputIndex, publicKey, signature) {
    const txData = tx.vin[inputIndex];
    const message = getTxInputMessage(tx, inputIndex);
    const publicKeyBuffer = Buffer.from(publicKey, "hex");
    const signatureBuffer = Buffer.from(signature, "hex");
    return secp256k1.ecdsaVerify(signatureBuffer, crypto.createHash("sha256").update(message).digest(), publicKeyBuffer);
}
// Construct the message to be signed for a transaction input
function getTxInputMessage(tx, inputIndex) {
    const txData = tx.vin[inputIndex];
    ...
    const scriptLength = Buffer.alloc(varIntBytes(txData.prevout.scriptpubkey.length / 2));
...
function validateMempoolTransactions(mempoolFolder) {
...
function validateTransaction(transactionData) {
    ...
    for (let i = 0; i < transactionData.vin.length; i++) {
        ...
        const script = input.scriptsig || input.witness.join(" ").split(" ").map((item) => parseInt(item, 16));
...
function varIntEncode(buffer, value, offset = 0) {
    if (value < 0xfd) {
        buffer[offset] = value;
        return 1;
    }
