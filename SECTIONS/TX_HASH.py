...
serialized_transaction = json.dumps(transaction_data, separators=(',', ':'), sort_keys=True).encode()


transaction_hash = hashlib.sha256(hashlib.sha256(serialized_transaction).digest()).digest()


reversed_hash = bytes(reversed(transaction_hash))


print(reversed_hash.hex())
